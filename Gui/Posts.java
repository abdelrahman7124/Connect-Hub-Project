...
public class Posts implements ActionListener, KeyListener {

    JFrame container = new JFrame("New post");
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == button[0]) {
            //add a new post
            try {
                Post post = ContentFactory.createPost(user.getId(), textfield.getText(), fileChooser.getSelectedFile().toString());
                contentManager.newPost(post);
                container.dispose();
                new NewsFeed(user);
            } catch (NullPointerException nullPointerException) {
                Post post = ContentFactory.createPost(user.getId(), textfield.getText(), "");
                contentManager.newPost(post);
                container.dispose();
                new NewsFeed(user);
            }
            ...
            fileChooser.setDialogTitle("Photo picking");
